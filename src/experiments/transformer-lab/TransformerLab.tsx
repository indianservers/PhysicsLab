    ...
    <div className="transformer-layout">
      ...
      <main className="transformer-main">
        ...
        <section className="transformer-stage" aria-label="Animated two-dimensional transformer apparatus">
          ...
          <button className="transformer-direct-turns" aria-label={`Drag secondary winding turns, ${input.secondaryTurns}`}
            onPointerDown={event=>event.currentTarget.setPointerCapture(event.pointerId)}
            onPointerMove={event=>{if(!event.currentTarget.hasPointerCapture(event.pointerId))return;const rect=event.currentTarget.getBoundingClientRect();update({secondaryTurns:20+Math.round(Math.max(0,Math.min(1,(event.clientY-rect.top)/rect.height))*295)*4})}}
            onKeyDown={event=>{if(event.key==="ArrowUp")update({secondaryTurns:input.secondaryTurns-4});if(event.key==="ArrowDown")update({secondaryTurns:input.secondaryTurns+4})}}><i style={{top:`${(input.secondaryTurns-20)/1180*100}%`}}/><span>DRAG Nₛ ↕</span></button>
