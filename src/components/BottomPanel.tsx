...
import { getMeasurementAdapter, graphableVariables, measurementUnavailable, serializeGraphPoint } from "../engine/measurementAdapters";

const tabs = ["Graphs", "Data table", "Formula", "Instruments", "Errors", "Steps", "Questions", "Lab report", "Log"];
const graphVariables: GraphVariable[] = [...graphableVariables];
...
export function BottomPanel() {
  ...
  const csv = () => {
    const metadataRows = [
      ["Generated By", "PhysicsLab Scientific Trust Export"],
      ["Model Class", "Dynamic Simulation"],
      ["Trust Level", String(Math.round(average(enabledTraces.map((trace) => trace.scientificConfidence ?? getMeasurementAdapter(trace.yKey)?.confidence ?? 0))))],
      ["Assumptions", uniqueAssumptions(enabledTraces).join("; ")],
      ["Formula Used", enabledTraces.map((trace) => `${trace.label}: ${getMeasurementAdapter(trace.yKey)?.name ?? measurementUnavailable(trace.yKey)}`).join("; ")],
      [],
    ];
    const header = graphVariables.map((key) => {
      const adapter = getMeasurementAdapter(key);
      return adapter ? `${key} (${adapter.unit || "unitless"})` : String(key);
    }).join(",");
    const rows = state.graphData.map((point) => graphVariables.map((key) => serializeCsvValue(serializeGraphPoint(point)[key])).join(","));
    download(new Blob([[...metadataRows.map((row) => row.map(serializeCsvValue).join(",")), header, ...rows].join("\n")], { type: "text/csv" }), "physicslab-data.csv");
  ...
  const json = () => {
    const payload = {
      ...
      trustLevel: Math.round(average(enabledTraces.map((trace) => trace.scientificConfidence ?? getMeasurementAdapter(trace.yKey)?.confidence ?? 0))),
      assumptions: uniqueAssumptions(enabledTraces),
      units: Object.fromEntries(graphVariables.map((key) => [key, getMeasurementAdapter(key)?.unit ?? "measurement unavailable"])),
      formulaUsed: enabledTraces.map((trace) => ({ trace: trace.label, measurement: getMeasurementAdapter(trace.yKey)?.name ?? measurementUnavailable(trace.yKey) })),
      data: state.graphData.map(serializeGraphPoint),
    };
    download(new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }), "physicslab-data.json");
    ...
    <section className="panel min-h-0 p-3">
      ...
      {activePanel === "Graphs" && (
        <div className={state.graphSplit ? "grid h-full gap-3 lg:grid-cols-1" : "grid gap-3 lg:grid-cols-[240px_minmax(0,1fr)_220px]"}>
          ...
          <div className="space-y-2 text-xs">
            {activeTrace && <GraphTrustCard trace={activeTrace} compact />}
...
function GraphTrustCard({ trace, compact = false }: { trace: GraphTraceConfig; compact?: boolean }) {
  const adapter = getMeasurementAdapter(trace.yKey);
  if (!adapter) {
    return (
      <div className="graph-trust-card warning">
        <strong>{measurementUnavailable(trace.yKey)}</strong>
      </div>
    );
  }
  return (
    <div className={compact ? "graph-trust-card compact" : "graph-trust-card"}>
      <div className="graph-trust-title">
        <strong>{adapter.name}</strong>
        <span>{adapter.symbol} · {adapter.unit || "unitless"}</span>
      </div>
      <div className="graph-trust-grid">
        <span>Source</span><b>{trace.sourceType ?? adapter.sourceModel}</b>
        <span>Trust</span><b>{trace.scientificConfidence ?? adapter.confidence}%</b>
        <span>Range</span><b>{adapter.validRange}</b>
      </div>
      {!compact && (
        <ul>
          {adapter.assumptions.slice(0, 3).map((assumption) => <li key={assumption}>{assumption}</li>)}
        </ul>
      )}
    </div>
  );
...
function drawPhasePortrait(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, data: GraphPoint[]) {
  ...
  const points = data
    .map((point) => ({ x: Number(point.x), y: Number(point.vx) }))
...
function unitForGraphKey(key: string) {
  return getMeasurementAdapter(key as GraphVariable)?.unit ?? "";
}
...
function ObservationTable() {
  const state = useLabStore();
  const activeTrace = state.graphTraces.find((trace) => trace.enabled);
  const xAdapter = activeTrace ? getMeasurementAdapter(activeTrace.xKey) : undefined;
  const yAdapter = activeTrace ? getMeasurementAdapter(activeTrace.yKey) : undefined;
  const rows = state.observationRows.length ? state.observationRows : activeTrace ? state.graphData.slice(-6).map((point, index) => ({
    id: `auto-${point.t}-${index}`,
    label: `Auto ${index + 1}`,
    measured: Number(point[activeTrace.yKey] ?? Number.NaN),
    expected: Number(point[activeTrace.xKey] ?? Number.NaN),
    unit: yAdapter?.unit ?? "",
    note: `auto-filled from ${yAdapter?.name ?? activeTrace.yKey} vs ${xAdapter?.name ?? activeTrace.xKey}`,
  })).filter((row) => Number.isFinite(row.measured) && Number.isFinite(row.expected)) : [];
...
function uniqueAssumptions(traces: GraphTraceConfig[]) {
  return [...new Set(traces.flatMap((trace) => trace.assumptions ?? getMeasurementAdapter(trace.yKey)?.assumptions ?? []))];
}
